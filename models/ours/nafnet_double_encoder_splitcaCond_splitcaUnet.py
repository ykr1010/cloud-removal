...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.intro = nn.Conv2d(in_channels=img_channel, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
        self.cond_intro = nn.Conv2d(in_channels=9, out_channels=width, kernel_size=3, padding=1, stride=1, groups=1,
                               bias=True)
    ...
    def forward(self, y_noisy, gammas, cond_img=None, **kwargs):
        t = self.map(self.emb(gammas.view(-1, )))
        y_noisy = self.check_image_size(y_noisy)
        y_cond = cond_img if cond_img is not None else kwargs.get('y_cond',None)
        assert y_cond is not None, 'Condition image must be provided as cond_img or y_cond'
        #
        y_cond = self.check_image_size(y_cond)

        x = self.intro(y_noisy)  # 3通道输入
        cond = self.cond_intro(y_cond)  # 3通道输入
        ...
        for decoder, up, enc_skip, cond_up in zip(self.decoders, self.ups, encs[::-1], self.cond_ups):
            x = up(x)
            cond = cond_up(cond)
            ...
            if cond.shape[1] != x.shape[1]:
                if cond.shape[1] > x.shape[1]:
                    ...
                    pad = x.shape[1] - cond.shape[1]
                    cond = F.pad(cond, (0,0,0,0,0,pad))
            x = x + cond + enc_skip
            x = decoder(x, t)
...
if __name__ == '__main__':
    ...
    # print(out_shape)
    # print(UNet())
    print(UNet()(*(torch.Tensor(1, 12, 256, 256), torch.ones(1,), torch.Tensor(1, 12, 256, 256))).shape)
    from thop import profile
    from thop import clever_format
    net = UNet()
    flops, params = profile(net, inputs=(torch.Tensor(1, 12, 256, 256), torch.ones(1,), torch.Tensor(1, 12, 256, 256)))
